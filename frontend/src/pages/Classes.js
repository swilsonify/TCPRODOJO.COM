...
const Classes = () => {
  ...
  // Parse a time string like "6:00 PM" into minutes from midnight for sorting
  ...
  // Build the flat list of classes per day for the current week.
  // Each entry = { classItem, dayName, time, date, isOneTime }
  const daySchedule = useMemo(() => {
    const result = {};

    classes.forEach(c => {
      if (c.is_one_time) {
        // One-time class: check if it falls in the current week
        ...
        // Recurring class: use schedule array, fall back to legacy fields
        let scheduleEntries = [];
  ...
  // Get only the days that have classes, in weekday order
  ...
  // Group entries by time within a day so same-time classes appear side-by-side
    ...
    <div className="pt-28 pb-20 px-4" data-testid="classes-page">
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="text-center mb-10">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white torture-text mb-4" data-testid="classes-title">CLASS SCHEDULE</h1>
        ...
        {/* Schedule Content */}
        {loading ? (
          <div className="text-center text-gray-400 py-16" data-testid="loading-indicator">Loading schedule...</div>
        ) : activeDays.length === 0 ? (
          <div className="text-center py-16" data-testid="no-classes-message">
            <p className="text-gray-400 text-lg mb-2">No classes scheduled this week.</p>
            <p className="text-gray-500 text-sm">Check back soon or navigate to another week.</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className={`px-5 py-3 flex items-center justify-between ${isToday ? 'bg-blue-600/20' : 'bg-gray-900/50'}`}>
                    ...
                    <span className="text-xs text-gray-500">{entries.length} class{entries.length !== 1 ? 'es' : ''}</span>
                  </div>
                  ...
                  <div className="p-4 space-y-3">
                    {timeGroups.map((group, gIdx) => (
                      <div
                        key={gIdx}
                        className={`flex flex-wrap gap-3 ${group.entries.length > 1 ? '' : ''}`}
                        ...
                        {group.entries.map((entry, eIdx) => {
                          ...
                          return (
                            <div
                              ...
                              title={
                                adminLoggedIn
                                  ? cancelled ? 'Right-click to restore' : 'Click to edit'
                            ...
                            >
                              ...
                              {entry.isOneTime && !cancelled && (
                                <div className="text-xs font-bold text-purple-300 mb-1">SPECIAL EVENT</div>
                              )}
                              ...
                              {!cancelled && (
                                <div className="flex items-center gap-2 mt-2">
                                  ...
                                  <span className="text-xs text-gray-500">{classItem.spots} spots</span>
                                </div>
                              )}

                              {/* Cancellation reason */}
                              {cancelled && cancellation?.reason && cancellation.reason !== 'No reason provided' && (
                                <p className="text-xs text-gray-500 mt-1 italic">Reason: {cancellation.reason}</p>
                              )}
                            </div>
                          );
                        })}
          ...
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" data-testid="edit-class-modal">
            <div className="bg-gray-900 border border-blue-500/20 rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-white">Manage Class</h3>
                <button
              ...
              <div className="bg-black/50 border border-blue-500/10 rounded-lg p-4 mb-6">
                ...
                <p className="text-sm text-gray-400">Instructor: {editingClass.instructor}</p>
              </div>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-white font-semibold mb-2 text-sm">Class Title</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-white font-semibold mb-2 text-sm">Instructor</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-white font-semibold mb-2 text-sm">Level</label>
                    <select
                  ...
                  <div>
                    <label className="block text-white font-semibold mb-2 text-sm">Available Spots</label>
                    <input
                      type="number"
                ...
                <div className="border-t border-blue-500/20 pt-4 mt-4">
                  <p className="text-gray-400 text-sm mb-3">
                    Actions for {selectedDate?.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}:
                  </p>
